...
const CaptureImages = () => {
    ...
    return (
        <div className="capture-page-images">
            <Navbar />
            <div className="capture-content-images">

                <div className="capture-card-images">
                    <h2 className="capture-title-images">Capture an image and detect text:</h2>
                    <hr className="capture-divider-images" />
                    {capturing ? (
                        <div className="camera-container-images">
                            <video ref={videoRef} autoPlay className="camera-feed-images" />
                            <div className="camera-buttons-images">
                                <button className="ct-btn-images" onClick={captureAndSend} disabled={loading}>
                                    {loading ? "analyzing..." : "Capture & Detect"}
                                </button>
                                <button className="ct-btn ct-btn-outline-images" onClick={stopCamera}>
                                    Cancel
                                </button>
                            </div>
                        </div>
                    ) : (
                        <button className="ct-btn-images" onClick={startCamera}>
                            Start
                        </button>
                    )}
                </div>

                <div className="capture-result-card-images">
                    <p className="capture-result-label-images">Result:</p>
                    <hr className="capture-divider-images" />
                    <p className="capture-result-images">
                        {loading ? "analyzing..." : result}
                    </p>
                </div>
